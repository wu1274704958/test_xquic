...
class relay_model{
    public:
    std::pair<RelayState,uint32_t> try_connect(const ::sockaddr& addr,const boost::uuids::uuid& token,std::weak_ptr<core::IStreamVariant> stream);
    std::weak_ptr<relay_pair> get_relay_pair(uint32_t id) const;
    bool remove_relay(uint32_t);
    bool remove_waiting(const ::sockaddr& addr,const boost::uuids::uuid& token);
    template<typename T>
    std::shared_ptr<T> find_other_peer_stream(uint32_t id,std::shared_ptr<core::IStreamVariant> self) const
    {
    ...
    protected:
    std::unordered_multimap<boost::uuids::uuid,peer_data> _waiting_map;
