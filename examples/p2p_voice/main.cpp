...
using P2PStreamType = core::StreamVariant<
	core::StreamVariantPair<1,VoiceStream>>;
...
protected:
	...
	std::weak_ptr<VoiceStream> p2p_stream;
...
int main(int argc, const char** argv)
{
	...
	try {
	    ...
	    stream->req_change<LobbyStream, mqas::tools::p2p::ReqRegistePeerPair>(msg);
	    auto lobby_stream = stream->get_holds_stream<LobbyStream>();
	    lobby_stream->on_change_helper = [stream,&ui](const mqas::tools::proto::p2p::ReqRespondPeerReqConnect& msg)
	    {
	        ...
	        ui.reg_helper_stream(stream->get_holds_stream<mqas::tools::p2p::P2PHelperClientStream>());
	    };
...
{
	auto self = this;
	std::function<void(std::shared_ptr<VoiceStream>)> func = [self](std::shared_ptr<VoiceStream> ptr)
	{
		self->p2p_stream = ptr;
	...
	if (is_server)
	{
		...
		{
			auto ptr = std::dynamic_pointer_cast<VoiceStream>(p);
			if(ptr)
				func(ptr); 
		});
	}
	...
	{
		LOG(INFO) << "req_change p2p voice";
		if (stream->req_change<VoiceStream>())
		{
			auto ptr = stream->get_holds_stream<VoiceStream>();
			if (ptr)
				func(ptr);
		}
	}
...
void tui::on_p2p_connected(bool success)
{
	...
	if (success)
	{
		...
		p2p_cxt->peer_name = none_str;
		std::optional<mqas::tools::proto::p2p::PeerData> peer = {};
		if (respond_change_helper && (peer = get_peer(respond_change_helper->peer_id())))
			p2p_cxt->peer_name = peer.value().name();
	}
	else
	{
		quit_p2p();
